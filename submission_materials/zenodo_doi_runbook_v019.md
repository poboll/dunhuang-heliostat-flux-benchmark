...
Status: GitHub release `v0.1.9` is the intended DOI-deposition source after the metadata/status correction pass.

## What this file is for

The manuscript cites the coordinate-source DOI `10.5281/zenodo.16957381` and the manuscript-facing GitHub release `v0.1.9`. The next required external archival step is to mint a new Zenodo DOI from the verified `v0.1.9` release. This release does not add experiments relative to `v0.1.7`; it removes pre-release wording from the archival metadata and keeps the DOI package aligned with the final author-review manuscript.

## Required order

1. Build and verify the final PDF and manifest.
2. Confirm the clean GitHub release `v0.1.9` is available from the final package.
3. Update `.zenodo.json`:
   - confirm `version` is `v0.1.9`;
   - confirm the GitHub `related_identifiers` URL points to release `v0.1.9`;
