...
Status: prepared for the manuscript-facing `v0.1.5` package.

## What this file is for

The manuscript cites the coordinate-source DOI `10.5281/zenodo.16957381` and the manuscript-facing GitHub release `v0.1.5`. The next required external archival step is to mint a new Zenodo DOI from the `v0.1.5` release.

## Required order

1. Build and verify the final PDF and manifest.
2. Confirm the clean GitHub release `v0.1.5` is available from the final package.
3. Update `.zenodo.json`:
   - confirm `version` is `v0.1.5`;
   - confirm the GitHub `related_identifiers` URL points to release `v0.1.5`;
