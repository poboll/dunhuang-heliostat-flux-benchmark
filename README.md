...
- Added a `v0.1.10` metadata/status correction pass for DOI deposition. This pass removes pre-release wording from the archival metadata and points Data/Code Availability to `v0.1.10`; it does not add experiments or broaden claims.
...
- Treat GitHub `v0.1.7` as the previous verified public release, and treat the current metadata/status-corrected package as the `v0.1.10` release target. Mint the Zenodo DOI from the exact release that matches the submitted PDF.
