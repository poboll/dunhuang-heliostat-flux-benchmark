...
- `latex/main.pdf`: compiled journal-style full-evidence inspection PDF, currently 40 pages in Elsevier `final,3p,times` mode with line numbers disabled for the formal inspection build. The detailed data dictionary, package-index table, and extended roadmap are kept in supplementary/manifest files rather than rendered as a technical-report appendix.
...
- Published the manuscript-facing public package as GitHub release `v0.1.5` (`fix: 同步v0.1.5清单与发布状态`) after the Figure 1 cleanup plus Zenodo metadata pass. The release archive is `solar_energy_elsarticle_v8_strict_review_rescue_20260622_v015.zip`, with the archive SHA-256 recorded in the companion `.sha256` release asset. The refreshed reproducibility manifest now records 612 checked files after stale-preview pruning and includes both `.zenodo.json` and `submission_materials/zenodo_metadata_v015.json`. The older Zenodo DOI remains only the coordinate-source DOI; a new Zenodo DOI for the V8 manuscript package is still required before final DOI-bearing journal upload.
...
- Added a 2026-06-22 Figure 1/DOI-metadata pass: rebuilt the lower locator/geometry panel while keeping the author-owned aerial photograph unedited, prepared `v0.1.5` Zenodo metadata, and refreshed the manifest so the DOI metadata is checksum-registered.
...
- Treat GitHub `v0.1.5` as the current manuscript-facing release. Before DOI-bearing journal upload, mint the Zenodo DOI from that release and write the DOI back into the manuscript and bibliography.
