...
- Published the manuscript-facing public package as GitHub release `v0.1.5` (`fix: 同步v0.1.5清单与发布状态`) after the Figure 1 cleanup plus Zenodo metadata pass. That release is now a historical checkpoint because the later journal-target, wording, README, and DOI-metadata consistency pass has been published as `v0.1.6`.
...
- Added and verified a `v0.1.10` metadata/status correction release for DOI deposition. This pass removes pre-release wording from the archival metadata and points Data/Code Availability to `v0.1.10`; it does not add experiments or broaden claims. The verified archive is `solar_energy_elsarticle_v8_strict_review_rescue_20260622_v010.zip`, SHA-256 `b4c5b850ab7b212775d93570d91d8669ce524d904f525314f8d1de2bdd4f6d2d`, with PDF SHA-256 `367d1a731b589bd455e6a47e9ad47a0e79550f8e8a9b4578ea19a2754403981a`.
...
- Treat GitHub `v0.1.10` as the verified DOI-deposition release for the current manuscript-facing package. Mint the Zenodo DOI from that exact release, then write the new DOI back into the manuscript, bibliography, metadata, and standalone data-availability statement.
