...
- Prepared the manuscript-facing public package for GitHub release `v0.1.3` (`fix: 优化图一与Zenodo元数据`) after the Figure 1 cleanup plus Zenodo metadata pass. The refreshed reproducibility manifest now records 612 checked files after stale-preview pruning and includes both `.zenodo.json` and `submission_materials/zenodo_metadata_v013.json`. The older Zenodo DOI remains only the coordinate-source DOI; a new Zenodo DOI for the V8 manuscript package is still required before final DOI-bearing journal upload.
