...
- Prepared the manuscript-facing public package for GitHub release `v0.1.3` (`fix: 优化图一与Zenodo元数据`, commit `386071d8b269338e0cb64c4c234ff5f0d977e2ec`) after the Figure 1 cleanup plus Zenodo metadata pass. The refreshed reproducibility manifest now records 612 checked files after stale-preview pruning and includes both `.zenodo.json` and `submission_materials/zenodo_metadata_v013.json`. The older Zenodo DOI remains only the coordinate-source DOI; a new Zenodo DOI for the V8 manuscript package is still required before final DOI-bearing journal upload.
...
- Added a 2026-06-22 Figure 1/DOI-metadata pass: rebuilt the lower locator/geometry panel while keeping the author-owned aerial photograph unedited, prepared `v0.1.3` Zenodo metadata, and refreshed the manifest so the DOI metadata is checksum-registered.
...
- Treat GitHub `v0.1.3` as the current manuscript-facing release. Before DOI-bearing journal upload, mint the Zenodo DOI from that release and write the DOI back into the manuscript and bibliography.
