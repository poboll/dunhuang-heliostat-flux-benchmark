...
- Manuscript-facing GitHub release recorded in the manuscript: <https://github.com/poboll/dunhuang-heliostat-flux-benchmark/releases/tag/v0.1.4>.
...
- Current local package status: the working package has been published as the `v0.1.4` manuscript-facing release, including Figure 1 lower-panel rebuild, LaTeX panel-label/caption cleanup, Zenodo metadata/runbook files, and a refreshed checksum manifest.
- Zenodo metadata draft: `.zenodo.json` has been prepared at the package root with `v0.1.4` metadata and the `v0.1.4` GitHub release URL. Add the minted DOI after Zenodo deposition.
- DOI runbook: `submission_materials/zenodo_doi_runbook_v014.md`.
...
- DOI metadata in manifest: the checksum manifest now includes both `.zenodo.json` and `submission_materials/zenodo_metadata_v014.json`.
...
1. Use the clean `v0.1.4` GitHub release as the manuscript-facing archive.
...
- The public GitHub repository is reachable as `poboll/dunhuang-heliostat-flux-benchmark`; `v0.1.4` is the manuscript-facing release for the current local package.
- The manuscript was rebuilt after `v0.1.1` to remove the rendered package-index table and improve page-level journal style. The `v0.1.4` package adds the Figure 1 and Zenodo-metadata cleanup on top of the earlier public release.
